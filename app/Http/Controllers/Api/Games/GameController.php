...
class GameController extends Controller
{
            ...
            if(auth('api')->check()) {
                ...
                if($wallet->total_balance > 0 || $game->id == 1580) {
                    ...
                    switch ($game->distribution) {
                        ...
                        case 'primeconnect':
                            // Permite somente os jogos da PG e Pragmatic para contas de influenciadores
                            if(!in_array($game->provider->code, ['PGSOFT', 'PRAGMATIC PLAY']) && auth('api')->user()->is_demo_agent === 1){
                                return response()->json(['error' => 'Acesso negado!', 'status' => false ], 400);
                            }
                            $gameLauncher = PrimeConnectTrait::gameLaunch($game);
                            if($gameLauncher) {
                                return response()->json([
                                'game' => $game,
     ...
     * @return mixed
     */
    public function webhookPrimeConnectMethod(Request $request)
    {
        return PrimeConnectTrait::webhook($request);
    }

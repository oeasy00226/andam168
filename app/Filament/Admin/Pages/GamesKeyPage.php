...
class GamesKeyPage extends Page implements HasForms
{
    ...
    protected static ?string $title = 'Chaves PrimeConnect';
    ...
    public function form(Form $form): Form
    {
        return $form
            ->schema([
                Section::make('API PRIMECONNECT')
                    ->description(new HtmlString('Para adquirir creditos acesse  <a href="https://apiconnect.primegames.com.br/" target="_blank" style="color: blue;">apiconnect.primegames.com.br</a>.'))
                    ->schema([
                        TextInput::make('primeconnect_code')
                            ->label('Agent Code')
                            ->placeholder('Digite aqui o Agent Code')
                            ->maxLength(191),
                        TextInput::make('primeconnect_token')
                            ->label('Agent Token')
                            ->placeholder('Digite aqui o Agent Token')
                            ->maxLength(191),
                        TextInput::make('primeconnect_secret_key')
                            ->label('Agent Secret')
